...
export default function App() {
  ...
  const [apiKey, setApiKey] = useState<string>('V_GEMINI_API_KEY_AI_STUDIO');
  ...
  const runTrainingSimulator = () => {
    ...
    setTrainingLogs([
      "🔄 Iniciando conexão com o modelo gemini-3.5-flash...",
            ...
            <div className="flex items-center gap-3 bg-[#080d19] border border-slate-900/40 px-3 py-1 rounded-xl">
              ...
              <div className="hidden md:block text-left">
                ...
                <p className="text-[8px] text-cyan-400 font-mono">
                  {modelStrategy === 'rotation' ? 'gemini-revezamento' : modelStrategy === 'fixo-lite' ? 'gemini-3.1-lite' : 'gemini-3.5-flash'}
                </p>
              </div>
            ...
            {activeTab === 'configuracoes' && (
              ...
              >
                <div className="bg-[#0b1120] border border-slate-900 rounded-3xl p-6 shadow-2xl space-y-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="text-xs text-slate-300 font-bold block mb-2">Chaves de Conexão Gemini (V_Gemini_API_Key & V2_Gemini_API_Key)</label>
                      <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] bg-cyan-950/40 text-cyan-400 font-mono border border-cyan-850 rounded px-2 py-0.5 shrink-0 w-44 text-center">Primária (V_Gemini_API_Key)</span>
                          <input
                            type="password"
                            value={apiKey}
                            ...
                            placeholder="Chave V_Gemini_API_Key"
                          />
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] bg-purple-950/40 text-purple-400 font-mono border border-purple-850 rounded px-2 py-0.5 shrink-0 w-44 text-center">Backup (V2_Gemini_API_Key)</span>
                          <input
                            type="password"
                            value="••••••••••••••••••••••••••••••••"
                            disabled
                            className="flex-1 bg-[#070b13]/40 border border-slate-850/60 rounded-xl px-4 py-2.5 text-xs text-slate-400 font-mono cursor-not-allowed"
                            placeholder="Configurada via Secrets no AI Studio"
                          />
                        </div>
                      </div>
                      <p className="text-[10px] text-slate-500 mt-2.5 leading-relaxed">
                        O motor DocEngine implementa um <strong>Circuito de Redundância e Failover em Cascata</strong>. Suas requisições utilizam primeiramente a chave <code>V_Gemini_API_Key</code>, rotacionando sequencialmente para a chave reserva <code>V2_Gemini_API_Key</code> caso o saldo pré-pago da primeira esteja esgotado (429), antes de transicionar automaticamente para o Groq (Llama-3.3-70b-versatile).
                      </p>
                    </div>
                    ...
                    <div className="border-t border-slate-850 pt-4">
                      ...
                      >
                        ...
                        <option value="fixo-flash">🎯 Fixo: gemini-3.5-flash (Modelo Principal)</option>
                        <option value="fixo-lite">⚡ Fixo: gemini-3.1-flash-lite (Modelo Econômico)</option>
                      </select>
                      <p className="text-[10px] text-slate-500 mt-1.5 leading-relaxed">
                        Selecione <strong>Revezamento de Modelos</strong> para alternar automaticamente a ordem de processamento do Gemini (gemini-3.5-flash, gemini-3.1-flash-lite, gemini-flash-latest, gemini-3.1-pro-preview) a cada requisição. Isso ajuda a utilizar as cotas e limites do Sandbox/Free Tier, evitando erros do tipo <code>RESOURCE_EXHAUSTED (429)</code> caso o saldo pré-pago de uma classe de modelos se esgote temporariamente.
                      </p>
                    </div>
